...
class Php
{
    ...
    public static function isCli()
    {
        ...
        if (!request()->server("SERVER_NAME") && !request()->server("HTTP_HOST")) {
            return true;
        }
        return false;
    }
    ...
    {
        return \App\Helpers\Carbon::createFromFormat("d M Y", static::$versionSupport[$majorMinor]["active"])->isFuture();
    }
    public static function hasSecurityPhpSupport($majorMinor)
    {
        return \App\Helpers\Carbon::createFromFormat("d M Y", static::$versionSupport[$majorMinor]["security"])->isFuture();
    }
